...
# Creating Image Subscriber Node
---
1. **Navigate to Your Package Directory**:
...
3. **Write the Subscriber Node Script**:
   - Open the `subscriber.py` file with a text editor:
   ...
   class ImageSubscriber(Node):
       ...
       def __init__(self):
           ...
           self.output_dir = os.path.join(
               '/home/tamuq/Downloads/New Pictures', #change this to the path of where you want your files to be saved once processed
               f"{month_name} {day}",
               f"Data_{timestamp}"
           )
...
4. ### _Important!!_ 
   Make sure that you modify the necessary lines in the code to write out the correct path to the files and folders!! (See comments in code for areas that require adjustment)
...
7. **Update `setup.py`**:
   ...
   entry_points={
       'console_scripts': [
           'publisher = system.scripts.publisher:main',
           'subscriber = system.scripts.subscriber:main',
       ],
   },
   ```

8. **Rebuild the Workspace**:
...
## **Run the Image Subscriber Node**:
   ...
   - You should see the following output for the publisher node:

   ```bash
   [INFO] [X.X] [mqtt_to_ros_publisher]: Subscribed to MQTT topic: camera/images
   ```

   - You should see the following output for the subscriber node (or similar... with different values and dates):

   ```bash
   2024-12-30 13:52:10.714049: I external/local_xla/xla/tsl/cuda/cudart_stub.cc:32] Could not find cuda drivers on your machine, GPU will not be used.
   2024-12-30 13:52:10.759794: I external/local_xla/xla/tsl/cuda/cudart_stub.cc:32] Could not find cuda drivers on your machine, GPU will not be used.
   2024-12-30 13:52:10.803319: E external/local_xla/xla/stream_executor/cuda/cuda_fft.cc:485] Unable to register cuFFT factory: Attempting to register factory for plugin cuFFT when one has already been registered
   2024-12-30 13:52:10.846531: E external/local_xla/xla/stream_executor/cuda/cuda_dnn.cc:8454] Unable to register cuDNN factory: Attempting to register factory for plugin cuDNN when one has already been registered
   2024-12-30 13:52:10.857640: E external/local_xla/xla/stream_executor/cuda/cuda_blas.cc:1452] Unable to register cuBLAS factory: Attempting to register factory for plugin cuBLAS when one has already been registered
   2024-12-30 13:52:10.921013: I tensorflow/core/platform/cpu_feature_guard.cc:210] This TensorFlow binary is optimized to use available CPU instructions in performance-critical operations.
   To enable the following instructions: AVX2 FMA, in other operations, rebuild TensorFlow with the appropriate compiler flags.
   2024-12-30 13:52:11.713272: W tensorflow/compiler/tf2tensorrt/utils/py_utils.cc:38] TF-TRT Warning: Could not find TensorRT
   WARNING: All log messages before absl::InitializeLog() is called are written to STDERR
   I0000 00:00:1735555932.499660    3842 cuda_executor.cc:1015] successful NUMA node read from SysFS had negative value (-1), but there must be at least one NUMA node, so returning NUMA node zero. See more at https://github.com/torvalds/linux/blob/v6.0/Documentation/ABI/testing/sysfs-bus-pci#L344-L355
   2024-12-30 13:52:12.500439: W tensorflow/core/common_runtime/gpu/gpu_device.cc:2343] Cannot dlopen some GPU libraries. Please make sure the missing libraries mentioned above are installed properly if you would like to use GPU. Follow the guide at https://www.tensorflow.org/install/gpu for how to download and setup the required libraries for your platform.
   Skipping registering GPU devices...
   ```

[Next step](https://github.com/hasanshomar/ROS2-Raspberry-Pi-HQ-Camera-Integration/blob/main/Setting%20Up%20the%20Raspberry%20Pi/1.%20Setting%20up%20Raspberry%20Pi%20OS.md)
